...
// provides methods that manage a set of available JDBC drive
import java.sql.DriverManager;
import java.sql.PreparedStatement;
// contains methods to access datbase values
// next() method moves down row of table
...
public class FlightsDB {
  ...
  public User logIn(String handle, String password) throws SQLException {
    ...
    String.format(
    "SELECT Top 1 name, userID, handle FROM Customer " +
    " WHERE Customer.password = ? AND" +
    " Customer.handle = ?", password, handle
    )
    );
    ...
    User user = new User(directResults.getInt("userID"),
                     directResults.getString("name"),
                     directResults.getString("handle")
                     );
    return user;            
    }
      return null;
    ...
    while (directResults.next()) {
      results.add(new Flight[] {
          new Flight(directResults.getInt("fid"), year, month, dayOfMonth,
              ...
              (int)directResults.getInt("actual_time"))
        });
    }
    directResults.close();
    
    PreparedStatement stmnt = conn.prepareStatement(String.format(
        ...
        stmnt.setInt(4, year);
        stmnt.setInt(5, month);
        stmnt.setInt(6, dayOfMonth);
        stmnt.setString(7, originCity);
        stmnt.setString(8, destCity);

    ResultSet twoHopResults = stmnt.executeQuery();    
  ...
  public List<Flight> getReservations(int userid) throws SQLException {
    ...
    PreparedStatement stmt = conn.prepareStatement(
    String.format(
    "SELECT fid, flight_num, origin_city, dest_city, actual_time, [year], month_id, day_of_month, carrier_id, [name] AS carrier_name " + 
    "FROM Flights f " +
    "LEFT JOIN Carriers c ON c.cid = f.carrier_id " +
    "WHERE f.fid IN( " +
        "SELECT fid FROM Itinerary i " +
        "WHERE [uID] = ?)")
    );
    stmt.setInt(1, userid);
    ResultSet directResults = stmt.executeQuery();
    while(directResults.next()) {
    Flight flight = new Flight(directResults.getInt("fid"), directResults.getInt("year"), directResults.getInt("month_id"),
                               directResults.getInt("day_of_month"), directResults.getString("carrier_name"),
  ...
  public int addReservations(
      ...
      throws SQLException {
/*    check flights to make sure it only has one flight
      get results of searching for other reservations on the same day. If results is empty, day is not full
      get results of sql query for Itinerary rows with the same flight id. If less than 3 in Flights[], you can continue
      commit adding of flight!
*/
      // checks to see if the user is trying to reserve multiple flights on the same day
      if (flights.size() > 1) {
         return RESERVATION_DAY_FULL;
      }
      // checks if the user has other flights on the same day
      List<Flight> currentRes = getReservations(userid);
      for (Flight i : currentRes) {
         if (i.month == month && i.year == year && i.dayOfMonth == dayOfMonth) {
            return RESERVATION_DAY_FULL;
         }
      }
      //gets a list of reservations for each flight and makes sure that none have more than 2 reservations
      beginTransaction();
      PreparedStatement stmt = conn.prepareStatement(
         String.format(
            "SELECT fID FROM Itinerary " +
            "WHERE fID = ?" 
         )
      );
      ...
      stmnt.setInt(2, flights.get(0).id);
      stmnt.executeUpdate();
      commitTransaction();
  ...
  public void removeReservations(int userid, List<Flight> flights) throws SQLException {
    //creating a list of fid's so that I can use it in the SQL statement
    int[] flight_ids = new int[flights.size()];
    for (int i = 0; i < flights.size(); i++) {
      flight_ids[i] = flights.get(i).id;
    }
    //SQL statement
    PreparedStatement stmt = conn.prepareStatement(
    String.format(
    ...
    //Iterating through the id's and deleting each row related to it
    beginTransaction();
    stmt.setInt(1, userid);
    for(int i = 0; i < flight_ids.length; i++) {
      stmt.setInt(2, flight_ids[i]);
      stmt.executeUpdate();
    }                
